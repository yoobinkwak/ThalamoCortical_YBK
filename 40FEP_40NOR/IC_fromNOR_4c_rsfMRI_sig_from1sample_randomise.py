...
def mergePerIC(voxelsize, side, smoothing, IC, stats):
    ...
    #randomise_dir = 'rsFC_Randomise_n80/NOR_only/{voxelsize}_{side}_{smoothing}_{IC}ICs/two_step_randomise/{stats}_tfce'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, stats=stats)
    randomise_dir = 'rsFC_Randomise_n80/NOR_only/{voxelsize}_{side}_{smoothing}_{IC}ICs/two_step_randomise/NOR_{stats}_tfce'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, stats=stats)
    ...
    for ic in ICs:
        for subj in subjects:
            #demeaned_dataFile = subj+'/IC_fromNOR_rsFC_ICA_1sample/{stats}_tfce/{voxelsize}_{side}_{smoothing}_{IC}ICs/dual_regression/t0.95_concatenated_{voxelsize}_{side}_{smoothing}_IC{ic}_demeaned_tfce_corrp_tstat1/dr_stage2_subject00000.nii.gz'.format(stats=stats, voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, ic=ic)
            demeaned_dataFile = subj+'/IC_fromNOR_rsFC_ICA_1sample/NOR_{stats}_tfce/{voxelsize}_{side}_{smoothing}_{IC}ICs/dual_regression/t0.95_concatenated_{voxelsize}_{side}_{smoothing}_IC{ic}_demeaned_tfce_corrp_tstat1/dr_stage2_subject00000.nii.gz'.format(stats=stats, voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, ic=ic)
            #z_demeaned_dataFile = subj+'/IC_fromNOR_rsFC_ICA_1sample/{stats}_tfce/{voxelsize}_{side}_{smoothing}_{IC}ICs/dual_regression/t0.95_concatenated_{voxelsize}_{side}_{smoothing}_IC{ic}_demeaned_tfce_corrp_tstat1/dr_stage2_subject00000_Z.nii.gz'.format(stats=stats, voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, ic=ic)
            z_demeaned_dataFile = subj+'/IC_fromNOR_rsFC_ICA_1sample/NOR_{stats}_tfce/{voxelsize}_{side}_{smoothing}_{IC}ICs/dual_regression/t0.95_concatenated_{voxelsize}_{side}_{smoothing}_IC{ic}_demeaned_tfce_corrp_tstat1/dr_stage2_subject00000_Z.nii.gz'.format(stats=stats, voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, ic=ic)

            if os.path.isfile(demeaned_dataFile):
                imgInputs_demean.append(demeaned_dataFile)

            if os.path.isfile(z_demeaned_dataFile):
                imgInputs_z_demean.append(z_demeaned_dataFile)
    ...
    for ic in ICs:
        ...
        if os.path.isfile(sig_cluster_demeaned):
            ...
            merged_subjs_perIC_z_demean = join(randomise_input_dir, 'Z_concatenated_{voxelsize}_{side}_{smoothing}_IC{ic}_demeaned.nii.gz'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, ic=ic))
            if not os.path.isfile(merged_subjs_perIC_z_demean):
                perSubj_perIC_z_demean = [x for x in imgInputs_z_demean if 'IC{ic}'.format(ic=ic) in x]
                acrossSubjs_perIC_z_demean = nilearn.image.concat_imgs(perSubj_perIC_z_demean)
                acrossSubjs_perIC_z_demean.to_filename(merged_subjs_perIC_z_demean)
...
def randomise(voxelsize, side, smoothing, IC, stats):
    ...
    inputs = [ x for x in os.listdir(randomise_input_dir) if x.startswith ('concatenated_')]
    ...
    z_inputs = [ x for x in os.listdir(randomise_input_dir) if x.startswith ('Z_')]
    for z_rand_input in z_inputs:
        z_rand_inputs = join(randomise_input_dir, z_rand_input)
        command = 'fslval {z_rand_inputs} dim4'.format(z_rand_inputs=z_rand_inputs)
        val = os.popen(command)
        a = val.read()
        b = a.strip()
        correct = '80'
        if b==correct:
            z_output_name = z_rand_inputs.split('/')[-1].split('.')[0]

        z_two_group_tfce_output_name = join(two_group_tfce_out_dir, '{z_output_name}'.format(z_output_name=z_output_name))
        z_two_group_tfce_output = join(two_group_tfce_out_dir, '{z_output_name}_tfce_corrp_tstat1.nii.gz'.format(z_output_name=z_output_name))
        if not os.path.isfile(z_two_group_tfce_output):
            mat = 'designs/2group_80n_design.mat'
            con = 'designs/2group_design.con'
            command = 'randomise -i {z_rand_inputs} -o {z_two_group_tfce_output_name} -d {mat} -t {con} -T --uncorrp'.format(z_rand_inputs=z_rand_inputs, z_two_group_tfce_output_name=z_two_group_tfce_output_name, mat=mat, con=con)
            os.popen(command).read
...
def threshold(voxelsize, side, smoothing, IC, stats):
    ...
    randomise_dir = 'rsFC_Randomise_n80/{voxelsize}_{side}_{smoothing}_{IC}ICs/two_step_randomise/NOR_{stats}_tfce'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, stats=stats)
...
def clusterInfo(voxelsize, side, smoothing, IC, stats):
    ...
    working_dir = 'rsFC_Randomise_n80/{voxelsize}_{side}_{smoothing}_{IC}ICs/two_step_randomise/NOR_{stats}_tfce/step2_2group_tfce'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, stats=stats)
...
def fslstatsV(voxelsize, side, smoothing, IC, stats):
    ...
    working_dir = 'rsFC_Randomise_n80/{voxelsize}_{side}_{smoothing}_{IC}ICs/two_step_randomise/NOR_{stats}_tfce/step2_2group_tfce/cluster_t0.95_corrp'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, stats=stats)
    imgs = [ x for x in os.listdir(working_dir) if 'nii.gz' in x]
