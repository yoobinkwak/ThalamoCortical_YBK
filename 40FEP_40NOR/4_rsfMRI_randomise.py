...
def mergePerIC(voxelsize, side, smoothing, IC):
    ...
    randomise_dir = 'rsFC_Randomise_n80/{voxelsize}_{side}_{smoothing}_{IC}ICs'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC)
    ...
    for subj in subjects:
        dataLoc = subj+'/DualRegression_TemporalICA/TemporalRegression_Stage2/{voxelsize}_{side}_{smoothing}_{IC}ICs'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC)
...
def randomise(voxelsize, side, smoothing, IC):
    ...
    one_sample_cluster_out_dir = join(randomise_dir, '1_sample_cluster')
    if not os.path.exists(one_sample_cluster_out_dir):
        os.mkdir(one_sample_cluster_out_dir)

    nocov_two_group_cluster_out_dir = join(randomise_dir, 'nocov_2_group_cluster')
    if not os.path.exists(nocov_two_group_cluster_out_dir):
        os.mkdir(nocov_two_group_cluster_out_dir)
    ...
    one_sample_voxel_out_dir = join(randomise_dir, '1_sample_voxel')
    if not os.path.exists(one_sample_voxel_out_dir):
        os.mkdir(one_sample_voxel_out_dir)

    nocov_two_group_voxel_out_dir = join(randomise_dir, 'nocov_2_group_voxel')
    if not os.path.exists(nocov_two_group_voxel_out_dir):
        os.mkdir(nocov_two_group_voxel_out_dir)
    ...
    for rand_input in inputs:
        ...
        one_sample_cluster_output_name = join(one_sample_cluster_out_dir, '{output_name}'.format(output_name=output_name))
        one_sample_cluster_output = join(one_sample_cluster_out_dir, '{output_name}_clustere_corrp_tstat1.nii.gz'.format(output_name=output_name))
        if not os.path.isfile(one_sample_cluster_output):
            mat = 'designs/1group_80n_design.mat'
            con = 'designs/1group_nocov_design.con'
            command = 'randomise -i {rand_inputs} -o {one_sample_cluster_output_name} -d {mat} -t {con} -c 2 --uncorrp'.format(rand_inputs=rand_inputs, one_sample_cluster_output_name=one_sample_cluster_output_name, mat=mat, con=con)
            os.popen(command).read

        one_sample_voxel_output_name = join(one_sample_voxel_out_dir, '{output_name}'.format(output_name=output_name))
        one_sample_voxel_output = join(one_sample_voxel_out_dir, '{output_name}_vox_corrp_tstat1.nii.gz'.format(output_name=output_name))
        if not os.path.isfile(one_sample_voxel_output):
            mat = 'designs/1group_80n_design.mat'
            con = 'designs/1group_nocov_design.con'
            command = 'randomise -i {rand_inputs} -o {one_sample_voxel_output_name} -d {mat} -t {con} -x --uncorrp'.format(rand_inputs=rand_inputs, one_sample_voxel_output_name=one_sample_voxel_output_name, mat=mat, con=con)
            os.popen(command).read
        ...
        nocov_two_group_cluster_output_name = join(nocov_two_group_cluster_out_dir, '{output_name}'.format(output_name=output_name))
        nocov_two_group_cluster_output = join(nocov_two_group_cluster_out_dir, '{output_name}_clustere_corrp_tstat1.nii.gz'.format(output_name=output_name))
        if not os.path.isfile(nocov_two_group_cluster_output):
            mat = 'designs/2group_80n_nocov_design.mat'
            con = 'designs/2group_nocov_design.con'
            command = 'randomise -i {rand_inputs} -o {nocov_two_group_cluster_output_name} -d {mat} -t {con} -c 2 --uncorrp'.format(rand_inputs=rand_inputs, nocov_two_group_cluster_output_name=nocov_two_group_cluster_output_name, mat=mat, con=con)
            os.popen(command).read


        nocov_two_group_voxel_output_name = join(nocov_two_group_voxel_out_dir, '{output_name}'.format(output_name=output_name))
        nocov_two_group_voxel_output = join(nocov_two_group_voxel_out_dir, '{output_name}_vox_corrp_tstat1.nii.gz'.format(output_name=output_name))
        if not os.path.isfile(nocov_two_group_voxel_output):
            mat = 'designs/2group_80n_nocov_design.mat'
            con = 'designs/2group_nocov_design.con'
            command = 'randomise -i {rand_inputs} -o {nocov_two_group_voxel_output_name} -d {mat} -t {con} -x --uncorrp'.format(rand_inputs=rand_inputs, nocov_two_group_voxel_output_name=nocov_two_group_voxel_output_name, mat=mat, con=con)
            os.popen(command).read
