...
def nosmoothRandomise(voxelsize, side, IC):
    ...
    for rand_input in inputs:
        ...
        if b==correct:
            ...
            #print(output_name)
            one_sample_tfce_output = join(one_sample_tfce_out_dir, '{output_name}.nii.gz'.format(output_name=output_name))
            if not os.path.isfile(one_sample_tfce_output):
                command = 'randomise -i {rand_inputs} -o {one_sample_tfce_output} -1 -T --uncorrp'.format(rand_inputs=rand_inputs, one_sample_tfce_output=one_sample_tfce_output)
                os.popen(command).read
