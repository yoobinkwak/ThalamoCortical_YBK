    ...
    for ic in ICs:
        for subject in subjects:
            if subject.startswith('FEP'):
                ...
                FEP_dataFile = '/10_fdt_paths.nii.gz'
    ...
    for ic in ICs:
        for subject in subjects:
            if subject.startswith('NOR'):
                ...
                NOR_dataFile = '/10_fdt_paths.nii.gz'
                NORimgInputs.append(NOR_dataLoc+NOR_dataFile)


    for ic in ICs:
        averaged_perIC = join(out_dir, 'all_ave_{voxelsize}_{side}_{smoothing}_IC{ic}.nii.gz'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, ic=ic))
        if not os.path.isfile(averaged_perIC):
            perSubj_perIC = [x for x in imgInputs if '{ic}'.format(ic=ic) in x]
            ave_perIC = nilearn.image.mean_img(perSubj_perIC)
            ave_perIC.to_filename(averaged_perIC)
        FEP_averaged_perIC = join(out_dir, 'FEP_ave_{voxelsize}_{side}_{smoothing}_IC{ic}.nii.gz'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, ic=ic))
        if not os.path.isfile(FEP_averaged_perIC):
            FEP_perSubj_perIC = [x for x in FEPimgInputs if '{ic}'.format(ic=ic) in x]
        ...
        NOR_averaged_perIC = join(out_dir, 'NOR_ave_{voxelsize}_{side}_{smoothing}_IC{ic}.nii.gz'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, ic=ic))
        if not os.path.isfile(NOR_averaged_perIC):
            NOR_perSubj_perIC = [x for x in NORimgInputs if '{ic}'.format(ic=ic) in x]
