...
tractDir_MNI=${subj}/Tractography
...
if [ ! -e masks/${side_s}_thalamus_HOSC.nii.gz ] ; then
    fslroi ${FSLDIR}/data/atlases/HarvardOxford/HarvardOxford-sub-prob-2mm.nii.gz masks/lh_thalamus_HOSC.nii.gz 3 1
    fslroi ${FSLDIR}/data/atlases/HarvardOxford/HarvardOxford-sub-prob-2mm.nii.gz masks/rh_thalamus_HOSC.nii.gz 14 1
fi

mniThalROI_raw=masks/${side_s}_thalamus_HOSC_60.nii.gz
mniThalROI=${dtiDir}/${side_s}_thalamus_DTI_HO_check.nii.gz 
if [ ! -e ${mniThalROI_raw} ]
then
    fslmaths masks/${side_s}_thalamus_HOSC.nii.gz -thr 60 -bin masks/${side_s}_thalamus_HOSC_60.nii.gz
fi
...
reconImg4s=${tractDir_MNI}/${side}/fdt_matrix2_reconstructed_4s
reconImg_ds3_4s=${tractDir_MNI}/${side}/fdt_matrix2_reconstructed_ds3_4s
...
## smoothing (NEED TO CHECK)
#if [ ! -e ${reconImg4s}.nii.gz ] ; then
...
if [ ! -e ${reconImg_ds3} ] ; then
    echo "Creating ${reconImg_ds3}"
    flirt -in ${reconImg} -ref ${reconImg} -applyisoxfm 3 -out ${reconImg_ds3}
    #-interp nearestneighbour \
fi

if [ ! -e ${reconImg_ds4} ] ; then
    echo "Creating ${reconImg_ds4}"
    flirt -in ${reconImg} -ref ${reconImg} -applyisoxfm 4 -out ${reconImg_ds4}
        #-interp nearestneighbour \
fi

#if [ ! -e ${reconImg_ds3_4s}.nii.gz ]
#then
