...
def postMelodicGLM (subject, side, hemi, downsample, spatialsmooth):
    ...
    melodicDir='wmSC_ICA_mICA/{downsample}_{side}_{spatialsmooth}_20ICs/dim20'.format(downsample=downsample, side=side, spatialsmooth=spatialsmooth)
    ...
    subject_map_dir ='wmSC_ICA_mICA/{downsample}_{side}_{spatialsmooth}_20ICs/masked_input'.format(downsample=downsample, side=side, spatialsmooth=spatialsmooth)
    log = 'subjects.txt'
    ...
    spatial_dir = join(spatial, '{downsample}_{side}_{spatialsmooth}_20ICs'.format(downsample=downsample, side=side, spatialsmooth=spatialsmooth))
    ...
    pattern_dir = join(pattern, '{downsample}_{side}_{spatialsmooth}_20ICs'.format(downsample=downsample, side=side, spatialsmooth=spatialsmooth))
    ...
    temporal_dir = join(temporal, '{downsample}_{side}_{spatialsmooth}_20ICs'.format(downsample=downsample, side=side, spatialsmooth=spatialsmooth))
...
def nosmoothPostMelodicGLM (subject, side, hemi, downsample):
    ...
    melodicDir='wmSC_ICA_mICA/{downsample}_{side}_20ICs/dim20'.format(downsample=downsample, side=side)
    mmthreshDir = join(melodicDir, 'stats')
    ...
    spatial_dir = join(spatial, '{downsample}_{side}_nosmooth_20ICs'.format(downsample=downsample, side=side))
    if not os.path.exists(spatial_dir):
        print('\ creating spatial regression directory for ' + subject)
        os.mkdir(spatial_dir)

    #pattern_dir = join(pattern, '{downsample}_{side}_nosmooth'.format(downsample=downsample, side=side))
    pattern_dir = join(pattern, '{downsample}_{side}_nosmooth_20ICs'.format(downsample=downsample, side=side))
    if not os.path.exists(pattern_dir):
        print('\ creating wmSC pattern directory for ' + subject)
        os.mkdir(pattern_dir)
    
    #temporal_dir = join(temporal, '{downsample}_{side}_nosmooth'.format(downsample=downsample, side=side))
    temporal_dir = join(temporal, '{downsample}_{side}_nosmooth_20ICs'.format(downsample=downsample, side=side))
    if not os.path.exists(temporal_dir):
        print('\ creating temporal directory for ' + subject)
        os.mkdir(temporal_dir)
