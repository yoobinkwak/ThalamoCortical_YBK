...
def nosmoothMergePerIC(voxelsize, side, IC):
    ...
    #randomise_dir = join(Randomise_dir, '{voxelsize}_{side}_nosmooth_{IC}ICs_bp2mni'.format(voxelsize=voxelsize, side=side, IC=IC))
    ...
    for subj in subjects:
        ...
        dataLoc = subj+'/DualRegression_TemporalICA/TemporalRegression_Stage2/{voxelsize}_{side}_nosmooth_{IC}ICs'.format(voxelsize=voxelsize, side=side, IC=IC)
...
def threshold_95(voxelsize, side, IC):
    ...
    #one_sample_tfce_out_dir = join(randomise_dir, '1_sample_tfce')
    #two_group_tfce_out_dir = join(randomise_dir, '2_group_tfce')
    one_sample_tfce_out_dir = join(randomise_dir, 'nocov_1_sample_tfce')
    two_group_tfce_out_dir = join(randomise_dir, 'nocov_2_group_tfce')
...
def SmoothMergePerIC(voxelsize, side, IC):
    
    Randomise_dir = 'rsFC_Randomise_n80'
    if not os.path.exists(Randomise_dir):
        os.mkdir(Randomise_dir)

    randomise_dir = join(Randomise_dir, '{voxelsize}_{side}_fwhm6_{IC}ICs'.format(voxelsize=voxelsize, side=side, IC=IC))
    if not os.path.exists(randomise_dir):
        os.mkdir(randomise_dir)
    ...
    melodicIC_map = nb.load('rsFC_ICA_cmd/{voxelsize}_{side}_fwhm6_{IC}ICs_masked/melodic_IC.nii.gz'.format(voxelsize=voxelsize, side=side, IC=IC))
    componentNum = melodicIC_map.shape[3]
    ...
    for subj in subjects:
        dataLoc = subj+'/DualRegression_TemporalICA/TemporalRegression_Stage2/{voxelsize}_{side}_fwhm6_{IC}ICs'.format(voxelsize=voxelsize, side=side, IC=IC)
    ...
    for ic in ICs:
        merged_subjs_perIC = join(randomise_input_dir, 'concatenated_{voxelsize}_{side}_fwhm6_IC{ic}.nii.gz'.format(voxelsize=voxelsize, side=side, ic=ic))
        if not os.path.isfile(merged_subjs_perIC):
           perSubj_perIC = [x for x in imgInputs if '00{ic}'.format(ic=ic) in x]
           print(perSubj_perIC)
           acrossSubjs_perIC = nilearn.image.concat_imgs(perSubj_perIC)
        ...
        merged_subjs_perIC_demean = join(randomise_input_dir, 'concatenated_{voxelsize}_{side}_fwhm6_IC{ic}_demeaned.nii.gz'.format(voxelsize=voxelsize, side=side, ic=ic))
        if not os.path.isfile(merged_subjs_perIC_demean):
           perSubj_perIC_demean = [x for x in imgInputs_demean if '00{ic}'.format(ic=ic) in x]
           #print(perSubj_perIC_demean)
           acrossSubjs_perIC_demean = nilearn.image.concat_imgs(perSubj_perIC_demean)
...
def SmoothRandomise(voxelsize, side, IC):
...
def PreprocSmoothMergePerIC(voxelsize, side, IC):
    
    Randomise_dir = 'rsFC_Randomise_n80'
    if not os.path.exists(Randomise_dir):
        os.mkdir(Randomise_dir)

    randomise_dir = join(Randomise_dir, '{voxelsize}_{side}_fwhm6preproc_{IC}ICs'.format(voxelsize=voxelsize, side=side, IC=IC))
    if not os.path.exists(randomise_dir):
        os.mkdir(randomise_dir)
    ...
    melodicIC_map = nb.load('rsFC_ICA_cmd/re_{voxelsize}_{side}_{IC}ICs_masked/melodic_IC.nii.gz'.format(voxelsize=voxelsize, side=side, IC=IC))
    componentNum = melodicIC_map.shape[3]
    ...
    for subj in subjects:
        dataLoc = subj+'/DualRegression_TemporalICA/TemporalRegression_Stage2/re_{voxelsize}_{side}_{IC}ICs'.format(voxelsize=voxelsize, side=side, IC=IC)
    ...
    for ic in ICs:
        merged_subjs_perIC = join(randomise_input_dir, 'concatenated_{voxelsize}_{side}_fwhm6preproc_IC{ic}.nii.gz'.format(voxelsize=voxelsize, side=side, ic=ic))
        if not os.path.isfile(merged_subjs_perIC):
           perSubj_perIC = [x for x in imgInputs if '00{ic}'.format(ic=ic) in x]
           #print(perSubj_perIC)
           acrossSubjs_perIC = nilearn.image.concat_imgs(perSubj_perIC)
        ...
        merged_subjs_perIC_demean = join(randomise_input_dir, 'concatenated_{voxelsize}_{side}_fwhm6preproc_IC{ic}_demeaned.nii.gz'.format(voxelsize=voxelsize, side=side, ic=ic))
        if not os.path.isfile(merged_subjs_perIC_demean):
           perSubj_perIC_demean = [x for x in imgInputs_demean if '00{ic}'.format(ic=ic) in x]
           #print(perSubj_perIC_demean)
           acrossSubjs_perIC_demean = nilearn.image.concat_imgs(perSubj_perIC_demean)
...
def PreprocSmoothRandomise(voxelsize, side, IC):
...
def Smooth_threshold_95(voxelsize, side, IC):
...
def Smooth_check_slices(voxelsize, side, IC):
...
def PreprocSmooth_threshold_95(voxelsize, side, IC):
...
def PreprocSmooth_check_slices(voxelsize, side, IC):
...
#        check_slices(args.voxelsize[0], args.side[0], args.IC[0])
        SmoothMergePerIC(args.voxelsize[0], args.side[0], args.IC[0])
        SmoothRandomise(args.voxelsize[0], args.side[0], args.IC[0])
        PreprocSmoothMergePerIC(args.voxelsize[0], args.side[0], args.IC[0])
        PreprocSmoothRandomise(args.voxelsize[0], args.side[0], args.IC[0])
        Smooth_threshold_95(args.voxelsize[0], args.side[0], args.IC[0])
        Smooth_check_slices(args.voxelsize[0], args.side[0], args.IC[0])
        PreprocSmooth_threshold_95(args.voxelsize[0], args.side[0], args.IC[0])
        PreprocSmooth_check_slices(args.voxelsize[0], args.side[0], args.IC[0])
