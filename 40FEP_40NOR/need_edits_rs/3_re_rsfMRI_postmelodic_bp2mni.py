...
def nosmoothFisherPostMelodicGLM (subject, side, sside, hemi, downsample, IC):
    ...
    spatial_dir = join(spatial, 're_{downsample}_{side}_{IC}ICs'.format(downsample=downsample, side=side, IC=IC))
    if not os.path.exists(spatial_dir):
        os.mkdir(spatial_dir)

    pattern_dir = join(pattern, 're_{downsample}_{side}_{IC}ICs'.format(downsample=downsample, side=side, IC=IC))
    if not os.path.exists(pattern_dir):
        os.mkdir(pattern_dir)
    
    temporal_dir = join(temporal, 're_{downsample}_{side}_{IC}ICs'.format(downsample=downsample, side=side, IC=IC))
    if not os.path.exists(temporal_dir):
        os.mkdir(temporal_dir)
