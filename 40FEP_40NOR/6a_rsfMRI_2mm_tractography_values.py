...
def fdt_normalize(voxelsize, side, smoothing, IC):
    ...
    log = 'subjects.txt'
    #log = 'test_subjects.txt'
    with open(log, 'r') as f:
        subjects = f.read().split()

    for subject in subjects:
        dataLoc = '{subject}/2mm_rsFC_seed_tractography/{voxelsize}_{side}_{smoothing}_{IC}ICs'.format(subject=subject, voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC)
        dataDirs = [ x for x in os.listdir(dataLoc) if x.startswith('2mm') and '_demeaned_' in x]
...
def extract_tract(voxelsize, side, smoothing, IC):
    ...
    for subject in subjects:
        ...
        for dataDir in dataDirs:
            ...
            ic = '{dataDir}'.format(dataDir=dataDir).split('_')[3]
            
            seedDir = '2mm_sig_clusters/{voxelsize}_{side}_{smoothing}_{IC}ICs/two_step_randomise/nocov_1_sample_tfce/sig_clusters'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC)
            seedFile_name = ''.join([ x for x in os.listdir(seedDir) if x.startswith('2mm') and '{ic}_demeaned_'.format(ic=ic) in x])
...
def extract_values(voxelsize, side, smoothing, IC):
    ...
    output_dir = '2mm_Tractography_Values'
