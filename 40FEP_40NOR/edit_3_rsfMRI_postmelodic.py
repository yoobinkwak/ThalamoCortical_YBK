...
def nonfisherPostMelodicGLM (subject, side, sside, hemi, downsample, IC):
    ...
    spatial_dir = join(spatial, '{downsample}_{side}_{IC}ICs_nonfisher'.format(downsample=downsample, side=side, IC=IC))
    if not os.path.exists(spatial_dir):
        print('\ creating spatial regression directory for ' + subject)
        os.mkdir(spatial_dir)

    pattern_dir = join(pattern, '{downsample}_{side}_{IC}ICs_nonfisher'.format(downsample=downsample, side=side, IC=IC))
    if not os.path.exists(pattern_dir):
        print('\ creating rsFC pattern directory for ' + subject)
        os.mkdir(pattern_dir)
    
    temporal_dir = join(temporal, '{downsample}_{side}_{IC}ICs_nonfisher'.format(downsample=downsample, side=side, IC=IC))
    ...
    for mmthreshImgss in mmthreshImgLoc:
        ...
        if not os.path.exists(stage_1):
            print(subject + 'running with nonfisher input dual regression stage 1 for ' + mmthreshImg)
        ...
        if not os.path.exists(pattern_stage_1):
            print(subject + 'running with nonfisher input dual regression stage 1 for rsFC pattern')
        ...
        if not os.path.exists(stage_1_demean):
            print(subject + 'running with nonfisher input dual regression stage 1 demean for ' + mmthreshImg)
        ...
        if not os.path.exists(pattern_stage_1_demean):
            print(subject + 'running with nonfisher input dual regression stage 1 demean for rsFC pattern')
        ...
        if not os.path.exists(z_stage_2):
            print(subject + 'running with nonfisher input glm stage 2 for ' + mmthreshImg)
            input_d = stage_1
        ...
        if not os.path.exists(z_pattern_stage_2):
            print(subject + 'running with nonfisher input glm stage 2 for rsFC pattern')
        ...
        if not os.path.exists(z_stage_2_demean):
            print(subject + 'running with nonfisher input glm stage 2 demean for ' + mmthreshImg)
        ...
        if not os.path.exists(z_pattern_stage_2_demean):
            print(subject + 'running with nonfisher input glm stage 2 demean for rsFC pattern')
...
def fisherPostMelodicGLM (subject, side, sside, hemi, downsample, IC):
    ...
    spatial_dir = join(spatial, '{downsample}_{side}_{IC}ICs_fisher'.format(downsample=downsample, side=side, IC=IC))
    if not os.path.exists(spatial_dir):
        print('\ creating spatial regression directory for ' + subject)
        os.mkdir(spatial_dir)

    pattern_dir = join(pattern, '{downsample}_{side}_{IC}ICs_fisher'.format(downsample=downsample, side=side, IC=IC))
    if not os.path.exists(pattern_dir):
        print('\ creating rsFC pattern directory for ' + subject)
        os.mkdir(pattern_dir)
    
    temporal_dir = join(temporal, '{downsample}_{side}_{IC}ICs_fisher'.format(downsample=downsample, side=side, IC=IC))
    if not os.path.exists(temporal_dir):
        print('\ creating temporal directory for ' + subject)
        os.mkdir(temporal_dir)
    ...
    for mmthreshImgss in mmthreshImgLoc:
        ...
        z_pattern_stage_2_demean = join(pattern_dir, 'z_{subject}_{mmthreshImg}_pattern_stage2_demeaned.nii.gz'.format(subject=subject, mmthreshImg=mmthreshImg))
        if not os.path.exists(z_pattern_stage_2_demean):
            print(subject + 'running with fisher input glm stage 2 demean for rsFC pattern')
            input_d = pattern_stage_1_demean
            command = 'fsl_glm -i {subject_map} -d {input_d} -o {pattern_stage_2_demean} --out_z={z_pattern_stage_2_demean} --demean'.format(subject_map=subject_map, input_d=input_d, thal=thal, pattern_stage_2_demean=pattern_stage_2_demean, z_pattern_stage_2_demean=z_pattern_stage_2_demean)
            os.popen(command).read()
...
if __name__== "__main__":
	...
	args = parser.parse_args()
        ...
        nonfisherPostMelodicGLM(args.subject[0], args.side[0], args.sside[0], args.hemi[0], args.downsample[0], args.IC[0])
        fisherPostMelodicGLM(args.subject[0], args.side[0], args.sside[0], args.hemi[0], args.downsample[0], args.IC[0])
