...
def fisherPostMelodicGLM (subject, side, sside, hemi, downsample, spatialsmooth, IC):
    ...
    thal = join(subject, 'REST/Preprocess/{hemi}_thal_on_bp_{downsample}_s3.nii.gz').format(hemi=hemi, downsample=downsample)  
...
def nosmoothFisherPostMelodicGLM (subject, side, sside, hemi, downsample, IC):
    ...
    mni = 'mni_brain_${downsample}.nii.gz'.format(downsample=downsample)
    thal = join(subject, 'REST/Preprocess/{hemi}_thal_on_bp_{downsample}.nii.gz').format(hemi=hemi, downsample=downsample)  
