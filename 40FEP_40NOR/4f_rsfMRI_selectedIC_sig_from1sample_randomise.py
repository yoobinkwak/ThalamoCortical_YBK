...
def mergePerIC(voxelsize, side, smoothing, IC, stats):
    ...
    randomise_dir = 'rsFC_Randomise_n80/{voxelsize}_{side}_{smoothing}_{IC}ICs/selected_ICs/two_step_randomise/{stats}_tfce'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, stats=stats)
    ...
    for ic in ICs:
        for subj in subjects:
            demeaned_dataFile = subj+'/rsFC_ICA_1sample/selected_ICs/{voxelsize}_{side}_{smoothing}_{IC}ICs/{stats}_tfce/dual_regression/t0.95_concatenated_{voxelsize}_{side}_{smoothing}_IC{ic}_demeaned_tfce_corrp_tstat1/dr_stage2_subject00000.nii.gz'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, stats=stats, ic=ic)
            undemeaned_dataFile = subj+'/rsFC_ICA_1sample/selected_ICs/{voxelsize}_{side}_{smoothing}_{IC}ICs/{stats}_tfce/dual_regression/t0.95_concatenated_{voxelsize}_{side}_{smoothing}_IC{ic}_tfce_corrp_tstat1/dr_stage2_subject00000.nii.gz'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, stats=stats, ic=ic)
...
def clusterInfo(voxelsize, side, smoothing, IC, stats):
    ...
    working_dir = 'rsFC_Randomise_n80/{voxelsize}_{side}_{smoothing}_{IC}ICs/selected_ICs/two_step_randomise/{stats}_tfce/step2_2group_tfce'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, stats=stats)
...
def fslstatsV(voxelsize, side, smoothing, IC, stats):
    ...
    working_dir = 'rsFC_Randomise_n80/{voxelsize}_{side}_{smoothing}_{IC}ICs/selected_ICs/two_step_randomise/{stats}_tfce/step2_2group_tfce/cluster_t0.95_corrp'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC, stats=stats)
    imgs = [ x for x in os.listdir(working_dir) if 'nii.gz' in x]
