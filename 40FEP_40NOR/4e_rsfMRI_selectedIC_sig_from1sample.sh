...
#for sub_dir in ${rand_dir}/ds3_bi_nosmooth_20ICs/selected_ICs
#do
#
...
for i in $@
do
    subj_dir=${i}/rsFC_ICA_1sample/selected_ICs/ds3_bi_nosmooth_20ICs
    if [ ! -d ${subj_dir} ] ; then
    ...
    do
        ...
        do
            ...
            echo ${clusters_name}
            if [ ! -d ${subj_dir}/1_sample_tfce/timeseries ] ; then
                mkdir -p ${subj_dir}/1_sample_tfce/timeseries 
            fi
            if [ ! -d ${subj_dir}/1_sample_tfce/dual_regression/${clusters_name} ] ; then
                mkdir -p ${subj_dir}/1_sample_tfce/dual_regression/${clusters_name} 
            fi
            fslmeants -i ${subj_map} -m ${clusters} -o ${subj_dir}/1_sample_tfce/timeseries/${clusters_name}
            dual_regression ${clusters} 0 -1 0 ${subj_dir}/1_sample_tfce/dual_regression/${clusters_name} ${subj_map}  
        done
        ...
        do
            ...
            echo ${clusters2_name}
            if [ ! -d ${subj_dir}/nocov_1_sample_tfce/timeseries ] ; then
                mkdir -p ${subj_dir}/nocov_1_sample_tfce/timeseries 
            fi
            if [ ! -d ${subj_dir}/nocov_1_sample_tfce/dual_regression/${clusters2_name} ] ; then
                mkdir -p ${subj_dir}/nocov_1_sample_tfce/dual_regression/${clusters2_name} 
            fi
            fslmeants -i ${subj_map} -m ${clusters2} -o ${subj_dir}/nocov_1_sample_tfce/timeseries/${clusters2_name}
            dual_regression ${clusters2} 0 -1 0 ${subj_dir}/nocov_1_sample_tfce/dual_regression/${clusters2_name} ${subj_map}  
        done
