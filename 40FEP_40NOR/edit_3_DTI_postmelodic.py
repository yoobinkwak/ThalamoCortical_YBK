...
def postMelodicGLM (subject, side, hemi, downsample, spatialsmooth):
    ...
    subject_dir = join(subject, 'Regression_tensorICA')
    ...
    spatial_dir = join(subject_dir, '{downsample}_{side}_{spatialsmooth}_spatial_regression'.format(downsample=downsample, side=side, spatialsmooth=spatialsmooth))
    if not os.path.exists(spatial_dir):
        print('\ creating spatial regression directory for ' + subject)
        os.mkdir(spatial_dir)

    pattern_dir = join(subject_dir, '{downsample}_{side}_{spatialsmooth}_wmSC_pattern'.format(downsample=downsample, side=side, spatialsmooth=spatialsmooth))
    if not os.path.exists(pattern_dir):
        print('\ creating wmSC pattern directory for ' + subject)
        os.mkdir(pattern_dir)
    
    temporal_dir = join(subject_dir, '{downsample}_{side}_{spatialsmooth}_temporal_regression'.format(downsample=downsample, side=side, spatialsmooth=spatialsmooth))
    ...
    for mmthreshImgss in mmthreshImgLoc:
        ...
        if not os.path.exists(pattern_stage_1_demean):
            print(subject + 'running dual regression stage 1 demean for wmSC pattern')
        ...
        if not os.path.exists(z_pattern_stage_2_demean):
            print(subject + 'running glm stage 2 demean for wmSC pattern')
...
def nosmoothPostMelodicGLM (subject, side, hemi, downsample):
    ...
    mni = 'mni_brain_${downsample}.nii.gz'.format(downsample=downsample)
    thal = 'masks/{hemi}_thalamus_HOSC_60{downsample}.nii.gz'.format(hemi=hemi, downsample=downsample)  
    ...
    spatial_dir = join(subject_dir, '{downsample}_{side}_spatial_regression'.format(downsample=downsample, side=side))
    if not os.path.exists(spatial_dir):
        print('\ creating spatial regression directory for ' + subject)
        os.mkdir(spatial_dir)

    pattern_dir = join(subject_dir, '{downsample}_{side}_wmSC_pattern'.format(downsample=downsample, side=side))
    if not os.path.exists(pattern_dir):
        print('\ creating wmSC pattern directory for ' + subject)
        os.mkdir(pattern_dir)
