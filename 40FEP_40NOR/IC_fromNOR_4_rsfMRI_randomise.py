...
def randomise(voxelsize, side, smoothing, IC):
    ...
    nocov_one_sample_cluster_out_dir = join(randomise_dir, 'nocov_1_sample_cluster')
    if not os.path.exists(nocov_one_sample_cluster_out_dir):
        os.mkdir(nocov_one_sample_cluster_out_dir)
    
    two_group_cluster_out_dir = join(randomise_dir, '2_group_cluster')
    if not os.path.exists(two_group_cluster_out_dir):
        os.mkdir(two_group_cluster_out_dir)


    nocov_one_sample_voxel_out_dir = join(randomise_dir, 'nocov_1_sample_voxel')
    if not os.path.exists(nocov_one_sample_voxel_out_dir):
        os.mkdir(nocov_one_sample_voxel_out_dir)
    
    two_group_voxel_out_dir = join(randomise_dir, '2_group_voxel')
    if not os.path.exists(two_group_voxel_out_dir):
        os.mkdir(two_group_voxel_out_dir)
    ...
    for rand_input in inputs:
        ...
        two_group_cluster_output_name = join(two_group_cluster_out_dir, '{output_name}'.format(output_name=output_name))
        two_group_cluster_output = join(two_group_cluster_out_dir, '{output_name}_clustere_corrp_tstat1.nii.gz'.format(output_name=output_name))
        if not os.path.isfile(two_group_cluster_output):
            mat = 'designs/2group_80n_design.mat'
            con = 'designs/2group_design.con'
            command = 'randomise -i {rand_inputs} -o {two_group_cluster_output_name} -d {mat} -t {con} -c 2 --uncorrp'.format(rand_inputs=rand_inputs, two_group_cluster_output_name=two_group_cluster_output_name, mat=mat, con=con)
            os.popen(command).read

        two_group_voxel_output_name = join(two_group_voxel_out_dir, '{output_name}'.format(output_name=output_name))
        two_group_voxel_output = join(two_group_voxel_out_dir, '{output_name}_vox_corrp_tstat1.nii.gz'.format(output_name=output_name))
        if not os.path.isfile(two_group_voxel_output):
            mat = 'designs/2group_80n_design.mat'
            con = 'designs/2group_design.con'
            command = 'randomise -i {rand_inputs} -o {two_group_voxel_output_name} -d {mat} -t {con} -x --uncorrp'.format(rand_inputs=rand_inputs, two_group_voxel_output_name=two_group_voxel_output_name, mat=mat, con=con)
            os.popen(command).read
...
def threshold(voxelsize, side, smoothing, IC):
    ...
    two_cluster_inputs = os.listdir(two_group_cluster_out_dir)
    two_voxel_inputs = os.listdir(two_group_voxel_out_dir)

    two_corrps = [x for x in two_inputs if '_corrp_' in x]
    one_corrps = [x for x in one_inputs if '_corrp_' in x]
    two_cluster_corrps = [x for x in two_cluster_inputs if '_corrp_' in x]
    two_voxel_corrps = [x for x in two_voxel_inputs if '_corrp_' in x]
    ...
    thr_corrp_two_group_cluster_dir = join(two_group_cluster_out_dir, 'cluster_t0.95_corrp')
    if not os.path.exists(thr_corrp_two_group_cluster_dir):
        os.mkdir(thr_corrp_two_group_cluster_dir)

    for two_cluster_corrp in two_cluster_corrps:
        two_cluster_corrp_data = join(two_group_cluster_out_dir, two_cluster_corrp)
        thr_corrp_two_group_cluster_output = join(thr_corrp_two_group_cluster_dir, 't0.95_{two_cluster_corrp}'.format(two_cluster_corrp=two_cluster_corrp))
        if not os.path.isfile(thr_corrp_two_group_cluster_output):
            command = 'cluster -i {two_cluster_corrp_data} -t 0.95 --othresh={thr_corrp_two_group_cluster_output}'.format(two_cluster_corrp_data=two_cluster_corrp_data, thr_corrp_two_group_cluster_output= thr_corrp_two_group_cluster_output)
            os.popen(command).read

    thr_corrp_two_group_voxel_dir = join(two_group_voxel_out_dir, 'cluster_t0.95_corrp')
    if not os.path.exists(thr_corrp_two_group_voxel_dir):
        os.mkdir(thr_corrp_two_group_voxel_dir)

    for two_voxel_corrp in two_voxel_corrps:
        two_voxel_corrp_data = join(two_group_voxel_out_dir, two_voxel_corrp)
        thr_corrp_two_group_voxel_output = join(thr_corrp_two_group_voxel_dir, 't0.95_{two_voxel_corrp}'.format(two_voxel_corrp=two_voxel_corrp))
        if not os.path.isfile(thr_corrp_two_group_voxel_output):
            command = 'cluster -i {two_voxel_corrp_data} -t 0.95 --othresh={thr_corrp_two_group_voxel_output}'.format(two_voxel_corrp_data=two_voxel_corrp_data, thr_corrp_two_group_voxel_output= thr_corrp_two_group_voxel_output)
            os.popen(command).read
...
def clusterInfo(voxelsize, side, smoothing, IC):
    ...
    working_dir = 'rsFC_Randomise_n80/NOR_only/{voxelsize}_{side}_{smoothing}_{IC}ICs/2_group_tfce'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC)
    #working_dir = 'rsFC_Randomise_n80/NOR_only/{voxelsize}_{side}_{smoothing}_{IC}ICs/2_group_cluster'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC)
    #working_dir = 'rsFC_Randomise_n80/NOR_only/{voxelsize}_{side}_{smoothing}_{IC}ICs/2_group_voxel'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC)
    #working_dir = 'rsFC_Randomise_n80/NOR_only/{voxelsize}_{side}_{smoothing}_{IC}ICs/nocov_1_sample_tfce'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC)
...
def fslstatsV(voxelsize, side, smoothing, IC):
    ...
    working_dir = 'rsFC_Randomise_n80/NOR_only/{voxelsize}_{side}_{smoothing}_{IC}ICs/2_group_tfce/cluster_t0.95_corrp'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC) 
    #working_dir = 'rsFC_Randomise_n80/NOR_only/{voxelsize}_{side}_{smoothing}_{IC}ICs/2_group_cluster/cluster_t0.95_corrp'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC)
    #working_dir = 'rsFC_Randomise_n80/NOR_only/{voxelsize}_{side}_{smoothing}_{IC}ICs/2_group_voxel/cluster_t0.95_corrp'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC)
    #working_dir = 'rsFC_Randomise_n80/NOR_only/{voxelsize}_{side}_{smoothing}_{IC}ICs/nocov_1_sample_tfce/cluster_t0.95_corrp'.format(voxelsize=voxelsize, side=side, smoothing=smoothing, IC=IC) 
