...
def nosmoothFisherPostMelodicGLM (subject, side, sside, hemi, downsample, IC):
    ...
    subject_dir = join(out_dir, 'fromNOR_ConcatICA')
    ...
    pattern_dir = join(specific_dir, 'rsFC_pattern')
    ...
    melodicDir='rsFC_ICA_cmd/NOR_only/NOR_{downsample}_{side}_{IC}ICs_masked'.format(downsample=downsample, side=side, IC=IC)
    ...
    if not os.path.exists(thresholded_pattern):
        print(subject + ' thresholding rsFC pattern for ' + mmthreshImg)
...
def fisherPostMelodicGLM (subject, side, sside, hemi, downsample, spatialsmooth, IC):
    ...
    melodicDir='rsFC_ICA_cmd/NOR_only/NOR_{downsample}_{side}_{spatialsmooth}_{IC}ICs_masked'.format(downsample=downsample, side=side, spatialsmooth=spatialsmooth, IC=IC)
...
def ReNosmoothFisherPostMelodicGLM (subject, side, sside, hemi, downsample, IC):
    ...
    melodicDir='rsFC_ICA_cmd/NOR_only/NOR_re_{downsample}_{side}_{IC}ICs_masked'.format(downsample=downsample, side=side, IC=IC)
    ...
    specific_dir = join(subject_dir, '{downsample}_{side}_fwhm6preproc_{IC}ICs'.format(downsample=downsample, side=side, IC=IC))
    if not os.path.exists(specific_dir):
        os.mkdir(specific_dir)
    ...
    thresholded_pattern = join(pattern_dir, 'z2.3_{subject}_pattern_stage2_demeaned.nii.gz'.format(subject=subject))
    if not os.path.exists(thresholded_pattern):
        print(subject + ' thresholding rsFC pattern for ' + melodicImg)
        command = 'fslmaths {z_pattern_stage_2_demean} -thr 2.3 {thresholded_pattern}'.format(z_pattern_stage_2_demean=z_pattern_stage_2_demean, thresholded_pattern=thresholded_pattern)
        os.popen(command).read()
