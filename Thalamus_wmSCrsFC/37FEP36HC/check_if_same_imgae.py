...
def randomiseCheckSame (side, voxelsize):
    ...
    diffs = []
    for ic in ICs:
        file1 = '37FEP36HC_randomise_rsFC/znorm_input/concatenated_{side}_ds{voxelsize}_IC{ic}.nii.gz'.format(side=side, voxelsize=voxelsize, ic=ic)
        file2 = 'OM2015_group_rsFC/input_group_rsFC/{side}_{voxelsize}mm_4fwhm_IC{ic}.nii.gz'.format(side=side, voxelsize=voxelsize, ic=ic)
        print('checking ' + file1)
        f1 = nb.load(file1)
        f2 = nb.load(file2)
        data1 = f1.get_data()
        ...
        file3 = '37FEP36HC_randomise_rsFC/znorm_output/tfce/fwe_corrected_p/{side}_ds{voxelsize}/concatenated_{side}_ds{voxelsize}_IC{ic}_tfce_corrp_tstat1.nii.gz'.format(side=side, voxelsize=voxelsize, ic=ic)
        file4 = 'OM2015_group_rsFC/output_group_rsFC/tfce/fwe_corrected_p/{side}_{voxelsize}mm_4fwhm/{side}_{voxelsize}mm_4fwhm_IC{ic}_tfce_corrp_tstat1.nii.gz'.format(side=side, voxelsize=voxelsize, ic=ic)
        print('checking ' + file3)
        f3 = nb.load(file3)
        f4 = nb.load(file4)
        data3 = f3.get_data()
        data4 = f4.get_data()
        ...
        file5 = '37FEP36HC_randomise_rsFC/znorm_output/tfce/fwe_corrected_p/{side}_ds{voxelsize}/concatenated_{side}_ds{voxelsize}_IC{ic}_tfce_corrp_tstat2.nii.gz'.format(side=side, voxelsize=voxelsize, ic=ic)
        file6 = 'OM2015_group_rsFC/output_group_rsFC/tfce/fwe_corrected_p/{side}_{voxelsize}mm_4fwhm/{side}_{voxelsize}mm_4fwhm_IC{ic}_tfce_corrp_tstat2.nii.gz'.format(side=side, voxelsize=voxelsize, ic=ic)
        print('checking ' + file5)
        f5 = nb.load(file5)
        f6 = nb.load(file6)
        data5 = f5.get_data()
        data6 = f6.get_data()
...
if __name__== "__main__":
	...
	args = parser.parse_args()
        ...
        randomiseCheckSame(args.side[0], args.voxelsize[0])
