...
out_dir=${subj}/37FEP36HC/${side}_ds${voxel_size}_thalamusICs_rsFC
if [ ! -e ${out_dir} ] ; then
    mkdir ${out_dir}
fi
...
in_dir=${subj}/37FEP36HC/${side}_ds${voxel_size}_mICA_dual_regression
...
outDIR=${subj}/37FEP36HC/${side}_ds${voxel_size}_nosmooth_thalamusICs_rsFC
if [ ! -e ${outDIR} ] ; then
    mkdir ${outDIR}
fi
...
inDIR=${subj}/37FEP36HC/${side}_ds${voxel_size}_nosmooth_mICA_dual_regression
numICs=`ls ${inDIR}/${subj}_thresh_zstat00*stage2.nii.gz | wc -l`

for Num in $(seq -w 01 ${NumICs})
do
    subj_ic_ts=${tsDIR}/${subj}_thresh_zstat00${Num}_ts
    if [ ! -e ${subj_ic_ts} ] ; then
        input=${inDIR}/${subj}_thresh_zstat00${Num}_stage2.nii.gz
        fsl_glm -i ${rs_ds3} -d ${input} -o ${subj_ic_ts}
        count_ts=`cat ${subj_ic_ts} | wc -l`
        correct_ts=112
        if [ ! ${count_ts} -eq ${correct_ts} ] ; then
    ...
    if [ ! -e ${z_subj_ic_ts} ] ; then
        z_input=${inDIR}/znorm_${subj}_thresh_zstat00${Num}_stage2.nii.gz
        fsl_glm -i ${rs_ds3} -d ${z_input} -o ${z_subj_ic_ts}
        z_count_ts=`cat ${z_subj_ic_ts} | wc -l`
        correct_ts=112
        if [ ! ${z_count_ts} -eq ${correct_ts} ] ; then
