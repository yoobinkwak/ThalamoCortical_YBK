...
def GroupAvePerIC(side, voxelsize):
    ...
    ave_dir = '37FEP36HC_averaged_wmSC'
    ...
    for subj in subjects:
        if subj.startswith('FEP'):
            FEP_dataLoc = subj+'/37FEP36HC/{side}_ds{voxelsize}_mICA_dual_regression/'.format(side=side, voxelsize=voxelsize)
            ICs=["%02d" % x for x in range(1,componentNum+1)]
            for ic in ICs:
                FEP_dataFile = '{subj}_thresh_zstat00{ic}_stage2.nii.gz'.format(subj=subj, ic=ic)
                FEPimgInputs.append(FEP_dataLoc+FEP_dataFile)
    NORimgInputs = []
    for subj in subjects:
        if subj.startswith('NOR'):
            NOR_dataLoc = subj+'/37FEP36HC/{side}_ds{voxelsize}_mICA_dual_regression/'.format(side=side, voxelsize=voxelsize)
            ICs=["%02d" % x for x in range(1,componentNum+1)]
            for ic in ICs:
                NOR_dataFile = '{subj}_thresh_zstat00{ic}_stage2.nii.gz'.format(subj=subj, ic=ic)
...
def znormGroupAvePerIC(side, voxelsize):
    ...
    for subj in subjects:
        if subj.startswith('FEP'):
            ...
            ICs=["%02d" % x for x in range(1,componentNum+1)]
            for ic in ICs:
                FEP_dataFile = 'znorm_{subj}_thresh_zstat00{ic}_stage2.nii.gz'.format(subj=subj, ic=ic)
                FEPimgInputs.append(FEP_dataLoc+FEP_dataFile)
    NORimgInputs = []
    for subj in subjects:
        if subj.startswith('NOR'):
            ...
            ICs=["%02d" % x for x in range(1,componentNum+1)]
            for ic in ICs:
                NOR_dataFile = 'znorm_{subj}_thresh_zstat00{ic}_stage2.nii.gz'.format(subj=subj, ic=ic)
                NORimgInputs.append(NOR_dataLoc+NOR_dataFile)
...
def nosmoothGroupAvePerIC(side, voxelsize):
    ...
    ave_dir = '37FEP36HC_averaged_wmSC_nosmooth'
    if not os.path.exists(ave_dir):
        os.mkdir(ave_dir)
    out_dir= join(ave_dir, '{side}_ds{voxelsize}_nosmooth'.format(side=side, voxelsize=voxelsize))
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
    ...
    imgInputs = []
    for subj in subjects:
        dataLoc = subj+'/37FEP36HC/{side}_ds{voxelsize}_nosmooth_mICA_dual_regression/'.format(side=side, voxelsize=voxelsize)
        ICs=["%02d" % x for x in range(1,componentNum+1)]
        for ic in ICs:
            dataFile = '{subj}_thresh_zstat00{ic}_stage2.nii.gz'.format(subj=subj, ic=ic)
            imgInputs.append(dataLoc+dataFile)
    ...
    for subj in subjects:
        if subj.startswith('FEP'):
            FEP_dataLoc = subj+'/37FEP36HC/{side}_ds{voxelsize}_nosmooth_mICA_dual_regression/'.format(side=side, voxelsize=voxelsize)
            ICs=["%02d" % x for x in range(1,componentNum+1)]
            for ic in ICs:
                FEP_dataFile = '{subj}_thresh_zstat00{ic}_stage2.nii.gz'.format(subj=subj, ic=ic)
                FEPimgInputs.append(FEP_dataLoc+FEP_dataFile)
    ...
    for subj in subjects:
        if subj.startswith('NOR'):
            NOR_dataLoc = subj+'/37FEP36HC/{side}_ds{voxelsize}_nosmooth_mICA_dual_regression/'.format(side=side, voxelsize=voxelsize)
            ICs=["%02d" % x for x in range(1,componentNum+1)]
            for ic in ICs:
                NOR_dataFile = '{subj}_thresh_zstat00{ic}_stage2.nii.gz'.format(subj=subj, ic=ic)
                NORimgInputs.append(NOR_dataLoc+NOR_dataFile)
