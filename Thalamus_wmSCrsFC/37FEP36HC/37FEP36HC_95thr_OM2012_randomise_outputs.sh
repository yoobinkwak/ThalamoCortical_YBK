#dir=37FEP36HC_randomise_wmSC/outputs/tfce
dir=37FEP36HC_randomise_wmSC/znorm_outputs/tfce
...
#v_dir=37FEP36HC_randomise_wmSC/outputs/voxel_wise
v_dir=37FEP36HC_randomise_wmSC/znorm_outputs/voxel_wise

v_fwe=${v_dir}/fwe_corrected_p
if [ ! -e ${v_fwe} ] ; then
    mkdir ${v_fwe}
    mkdir ${v_fwe}/left_ds3
    mkdir ${v_fwe}/right_ds3
    mkdir ${v_dir}/uncorrected_p
    mkdir ${v_dir}/uncorrected_p/left_ds3
    mkdir ${v_dir}/uncorrected_p/right_ds3
    mkdir ${v_dir}/raw_stats
    mv ${v_dir}/concatenated_left_ds3_IC*vox_corrp_tstat* ${v_fwe}/left_ds3/
    mv ${v_dir}/concatenated_right_ds3_IC*vox_corrp_tstat* ${v_fwe}/right_ds3/
    mv ${v_dir}/concatenated_left_ds3_IC*vox_p_tstat* ${v_dir}/uncorrected_p/left_ds3/
    mv ${v_dir}/concatenated_right_ds3_IC*vox_p_tstat* ${v_dir}/uncorrected_p/right_ds3/
    mv ${v_dir}/concatenated_*_tstat* ${v_dir}/raw_stats/

fi

v_left_ds3=${v_fwe}/left_ds3
v_l_dir=`basename ${v_left_ds3}`
v_l_dir_thr=${v_fwe}/95thr_${v_l_dir}
if [ ! -e ${v_l_dir_thr} ] ; then
    mkdir ${v_l_dir_thr}
fi
v_l_dir_thr_mc=${v_fwe}/mc_95thr_${v_l_dir}
if [ ! -e ${v_l_dir_thr_mc} ] ; then
    mkdir ${v_l_dir_thr_mc}
fi

l_mc_correct=`ls ${v_left_ds3}/*tstat1.nii.gz | wc -l`
sig_level=0.05
l_mc_corrected_sig=`echo "scale=6; 1 - (${sig_level} / ${l_mc_correct})" | bc -l`
echo ${l_mc_corrected_sig}

for l_tstat1 in ${v_left_ds3}/*tstat1.nii.gz
do
    base=`basename ${l_tstat1}`
    tstat1_out=${v_l_dir_thr}/95thr_${base}
    if [ ! -e ${tstat1_out} ] ; then
        fslmaths ${l_tstat1} -thr 0.95 ${tstat1_out}
    elif [ -e ${tstat1_out} ] ; then
        val=`fslstats ${tstat1_out} -V | head -n1 | sed -e 's/\s.*$//'`
        zero=0
        if [ ! ${val} -eq ${zero} ] ; then
            echo ${tstat1_out} >> ${v_fwe}/survive_95.txt
        fi
    fi

    tstat1_mc=${v_l_dir_thr_mc}/mc_95thr_${base}
    if [ ! -e ${tstat1_mc} ] ; then
        fslmaths ${l_tstat1} -thr ${l_mc_corrected_sig} ${tstat1_mc}
    elif [ -e ${tstat1_mc} ] ; then
        val=`fslstats ${tstat1_mc} -V | head -n1 | sed -e 's/\s.*$//'`
        zero=0
        if [ ! ${val} == ${zero} ] ; then
            echo ${tstat1_mc} >> ${v_fwe}/survive_mc_95.txt
        fi
    fi

done

for l_tstat2 in ${v_left_ds3}/*tstat2.nii.gz
do
    base=`basename ${l_tstat2}`
    tstat2_out=${v_l_dir_thr}/95thr_${base}
    if [ ! -e ${tstat2_out} ] ; then
        fslmaths ${l_tstat2} -thr 0.95 ${tstat2_out}
    elif [ -e ${tstat2_out} ] ; then
        val=`fslstats ${tstat2_out} -V | head -n1 | sed -e 's/\s.*$//'`
        zero=0
        if [ ! ${val} -eq ${zero} ] ; then
            echo ${tstat2_out} >> ${v_fwe}/survive_95.txt
        fi
    fi
    
    tstat2_mc=${v_l_dir_thr_mc}/mc_95thr_${base}
    if [ ! -e ${tstat2_mc} ] ; then
        fslmaths ${l_tstat2} -thr ${l_mc_corrected_sig} ${tstat2_mc}
    elif [ -e ${tstat2_mc} ] ; then
        val=`fslstats ${tstat2_mc} -V | head -n1 | sed -e 's/\s.*$//'`
        zero=0
        if [ ! ${val} == ${zero} ] ; then
            echo ${tstat2_mc} >> ${v_fwe}/survive_mc_95.txt
        fi
    fi

done

v_right_ds3=${v_fwe}/right_ds3
v_r_dir=`basename ${v_right_ds3}`
v_r_dir_thr=${v_fwe}/95thr_${v_r_dir}
if [ ! -e ${v_r_dir_thr} ] ; then
    mkdir ${v_r_dir_thr}
fi
v_r_dir_thr_mc=${v_fwe}/mc_95thr_${v_r_dir}
if [ ! -e ${v_r_dir_thr_mc} ] ; then
    mkdir ${v_r_dir_thr_mc}
fi

r_mc_correct=`ls ${v_right_ds3}/*tstat1.nii.gz | wc -l`
sig_level=0.05
r_mc_corrected_sig=`echo "scale=6; 1 - (${sig_level} / ${r_mc_correct})" | bc -l`
echo ${r_mc_corrected_sig}

for r_tstat1 in ${v_right_ds3}/*tstat1.nii.gz
do
    base=`basename ${r_tstat1}`
    tstat1_out=${v_r_dir_thr}/95thr_${base}
    if [ ! -e ${tstat1_out} ] ; then
        fslmaths ${r_tstat1} -thr 0.95 ${tstat1_out}
    elif [ -e ${tstat1_out} ] ; then
        val=`fslstats ${tstat1_out} -V | head -n1 | sed -e 's/\s.*$//'`
    ...
    tstat1_mc=${v_r_dir_thr_mc}/mc_95thr_${base}
    if [ ! -e ${tstat1_mc} ] ; then
        fslmaths ${r_tstat1} -thr ${r_mc_corrected_sig} ${tstat1_mc}
    elif [ -e ${tstat1_mc} ] ; then
        val=`fslstats ${tstat1_mc} -V | head -n1 | sed -e 's/\s.*$//'`
...
for r_tstat2 in ${v_right_ds3}/*tstat2.nii.gz
do
    base=`basename ${r_tstat2}`
    tstat2_out=${v_r_dir_thr}/95thr_${base}
    if [ ! -e ${tstat2_out} ] ; then
        fslmaths ${r_tstat2} -thr 0.95 ${tstat2_out}
    elif [ -e ${tstat2_out} ] ; then
        val=`fslstats ${tstat2_out} -V | head -n1 | sed -e 's/\s.*$//'`
    ...
    tstat2_mc=${v_r_dir_thr_mc}/mc_95thr_${base}
    if [ ! -e ${tstat2_mc} ] ; then
        fslmaths ${r_tstat2} -thr ${r_mc_corrected_sig} ${tstat2_mc}
    elif [ -e ${tstat2_mc} ] ; then
        val=`fslstats ${tstat2_mc} -V | head -n1 | sed -e 's/\s.*$//'`
