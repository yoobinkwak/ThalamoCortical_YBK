...
out_dir=${subj}/37FEP36HC/bin_${side}_ds${voxel_size}_thalamusICs_rsFC
if [ ! -e ${out_dir} ] ; then
    mkdir ${out_dir}
fi
...
pre_bin_dir=${subj}/37FEP36HC/${side}_ds${voxel_size}_mICA_dual_regression
in_dir=${subj}/37FEP36HC/bin_${side}_ds${voxel_size}_mICA_dual_regression
if [ ! -e ${in_dir} ] ; then
    mkdir ${in_dir}
fi


NumICs=`ls ${pre_bin_dir}/${subj}_thresh_zstat00*stage2.nii.gz | wc -l`

for num in $(seq -w 01 ${NumICs})
do
    pre_bin=${pre_bin_dir}/${subj}_thresh_zstat00${num}_stage2.nii.gz
    bin=${in_dir}/bin_${subj}_thresh_zstat00${num}_stage2.nii.gz
    if [ ! -e ${bin} ] ; then
        fslmaths ${pre_bin} -bin ${bin}
    fi
    z_pre_bin=${pre_bin_dir}/znorm_${subj}_thresh_zstat00${num}_stage2.nii.gz
    z_bin=${in_dir}/bin_znorm_${subj}_thresh_zstat00${num}_stage2.nii.gz
    if [ ! -e ${z_bin} ] ; then
        fslmaths ${z_pre_bin} -bin ${z_bin}
    fi
done
...
do
    ...
    if [ ! -e ${subj_ic_ts} ] ; then
        input=${in_dir}/bin_${subj}_thresh_zstat00${num}_stage2.nii.gz
    ...
    if [ ! -e ${z_subj_ic_ts} ] ; then
        z_input=${in_dir}/bin_znorm_${subj}_thresh_zstat00${num}_stage2.nii.gz
...
outDIR=${subj}/37FEP36HC/bin_${side}_ds${voxel_size}_nosmooth_thalamusICs_rsFC
if [ ! -e ${outDIR} ] ; then
    mkdir ${outDIR}
fi
...
PRE_BIN_DIR=${subj}/37FEP36HC/${side}_ds${voxel_size}_mICA_dual_regression
inDIR=${subj}/37FEP36HC/bin_${side}_ds${voxel_size}_mICA_dual_regression
if [ ! -e ${inDIR} ] ; then
    mkdir ${inDIR}
fi
...
do
    pre_bin=${PRE_BIN_DIR}/${subj}_thresh_zstat00${num}_stage2.nii.gz
    bin=${inDIR}/bin_${subj}_thresh_zstat00${num}_stage2.nii.gz
    if [ ! -e ${bin} ] ; then
        fslmaths ${pre_bin} -bin ${bin}
    fi
    z_pre_bin=${PRE_BIN_DIR}/znorm_${subj}_thresh_zstat00${num}_stage2.nii.gz
    z_bin=${inDIR}/bin_znorm_${subj}_thresh_zstat00${num}_stage2.nii.gz
    if [ ! -e ${z_bin} ] ; then
        fslmaths ${z_pre_bin} -bin ${z_bin}
    fi
done
...
do
    ...
    if [ ! -e ${subj_ic_ts} ] ; then
        input=${inDIR}/bin_${subj}_thresh_zstat00${Num}_stage2.nii.gz
        fsl_glm -i ${rs_ds3} -d ${input} -o ${subj_ic_ts}
        count_ts=`cat ${subj_ic_ts} | wc -l`
        correct_ts=112
        if [ ! ${count_ts} -eq ${correct_ts} ] ; then
    ...
    if [ ! -e ${z_subj_ic_ts} ] ; then
        z_input=${inDIR}/bin_znorm_${subj}_thresh_zstat00${Num}_stage2.nii.gz
        fsl_glm -i ${rs_ds3} -d ${z_input} -o ${z_subj_ic_ts}
        z_count_ts=`cat ${z_subj_ic_ts} | wc -l`
        correct_ts=112
        if [ ! ${z_count_ts} -eq ${correct_ts} ] ; then
