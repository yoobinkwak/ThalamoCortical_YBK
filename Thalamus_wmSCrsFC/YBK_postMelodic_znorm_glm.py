...
def zStats (melodicDir, subject):
	...
	np.savetxt(os.path.join(melodicDir, 'glm_out', 'znorm_{subject}_stage1_{mmthreshImg}').format(subject=subject, mmthreshImg=mmthreshImg), fsl_glm_mat_z)
        ...
        stage_1_mapss = [join(stage_1_dir, x) for x in os.listdir(stage_1_dir) if x.startswith('znorm_{subject}_stage1'.format(subject = subject))]
        for stage_1_mapLoc in stage_1_mapss:
            stage_1_maps = basename(stage_1_mapLoc)
            stage_1_map = stage_1_maps.split('.')[0] 
