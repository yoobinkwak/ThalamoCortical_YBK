...
def zStats (melodicDir, subject):
	...
	np.savetxt(os.path.join(melodicDir, 'glm_out', 'znorm_{subject}_stage1_{mmthreshImg}').format(subject=subject, mmthreshImg=mmthreshImg), fsl_glm_mat_z)
        ...
        for stage_1_map in stage_1_maps:
            ...
            command = 'fsl_glm -i {subject_map} -d {stage_1_map} -m /Volume/CCNC_W1_2T/Thalamus_SCrsFC_20171026/lh_thalamus_HOSC_60_ds3.nii.gz -o {stage_2_out}'.format(subject_map = subject_map, stage_1_map = stage_1_map, stage_2_out = stage_2_out)
