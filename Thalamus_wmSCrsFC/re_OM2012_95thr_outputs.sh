...
dir_thr=re_OM2012_tensor_stats/outputs/tfce/fwe_corrected_p/95thr_${input}
if [ ! -e ${dir_thr} ] ; then
    mkdir ${dir_thr}
fi

dir_thr_mc=re_OM2012_tensor_stats/outputs/tfce/fwe_corrected_p/mc_95thr_${input}
if [ ! -e ${dir_thr_mc} ] ; then
    mkdir ${dir_thr_mc}
fi

dir_in=re_OM2012_tensor_stats/outputs/tfce/fwe_corrected_p/${input}
...
do
    ...
    if [ ! -e ${tstat1_out} ] ; then
        ...
        zero=0
        if [ ! ${val} -eq ${zero} ] ; then
            echo ${tstat1_out} >> re_OM2012_tensor_stats/outputs/tfce/fwe_corrected_p/survive_95.txt
        fi
    fi
    ...
    if [ ! -e ${tstat1_mc} ] ; then
        ...
        zero=0
        if [ ! ${val} == ${zero} ] ; then
            echo ${tstat1_mc} >> re_OM2012_tensor_stats/outputs/tfce/fwe_corrected_p/survive_mc_95.txt
        fi
    fi
...
do
    ...
    if [ ! -e ${tstat2_out} ] ; then
        ...
        zero=0
        if [ ! ${val} -eq ${zero} ] ; then
            echo ${tstat2_out} >> re_OM2012_tensor_stats/outputs/tfce/fwe_corrected_p/survive_95.txt
        fi
    fi
    ...
    if [ ! -e ${tstat2_mc} ] ; then
        ...
        zero=0
        if [ ! ${val} == ${zero} ] ; then
            echo ${tstat2_mc} >> re_OM2012_tensor_stats/outputs/tfce/fwe_corrected_p/survive_mc_95.txt
        fi
    fi
