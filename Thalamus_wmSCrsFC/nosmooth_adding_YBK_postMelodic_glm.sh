...
thal=/Volume/CCNC_W1_2T/Thalamus_SCrsFC_20171026/${hemi}_thalamus_HOSC_60.nii.gz
...
if [ ! -e ${glm_stage1} ] ; then
    ...
    if [ ! ${downsample} == '0' ] ; then
        ...
        fsl_glm -i ${subject_map} -d ${melodic_ic} -m ${thal} -o ${glm_stage1}
    fi
else
...
if [ ! -e ${glm_stage2}.nii.gz ] ; then
    ...
    if [ ! ${downsample} == '0' ] ; then
        ...
        fsl_glm -i ${subject_map} -d ${glm_stage1} -m ${thal} -o ${glm_stage2}
    fi
else
...
do
	...
	if [ ! -e ${glm_stage1}_${outputname} ] ; then
        ...
        if [ ! ${downsample} == '0' ] ; then
	        ...
	        fsl_glm -i ${subject_map} -d ${map} -m ${thal} -o ${glm_stage1}_${outputname}
        fi
    else
	...
	if [ ! -e ${glm_stage2}_${outputname}.nii.gz ] ; then
        ...
        if [ ! ${downsample} == '0' ] ; then
	        ...
	        fsl_glm -i ${subject_map} -d ${glm_stage1}_${outputname} -m ${thal} -o ${glm_stage2}_${outputname} 	
        fi
    else
