...
if [ ! -e ${lh_glm_out_dir} ] ; then
	mkdir ${lh_glm_out_dir}
fi
...
do
	...
	for l_map in ${lh_melodic_dir}/stats/thresh_zstat*
	do
		base=`basename ${l_map}`
		outputname=`remove_ext ${base}`
		fsl_glm -i ${lh_subject_l_map} -d ${l_map} -o ${lh_glm_stage1}_${outputname} 
		fsl_glm -i ${lh_subject_l_map} -d ${lh_glm_stage1}_${outputname} -m ${lh_thal_ds3} -o ${lh_glm_stage2}_${outputname} 
	done
done
...
if [ ! -e ${rh_glm_out_dir} ] ; then
	mkdir ${rh_glm_out_dir}
fi
...
do
	...
	for r_map in ${rh_melodic_dir}/stats/thresh_zstat*
	do
		base=`basename ${r_map}`
		outputname=`remove_ext ${base}`
		fsl_glm -i ${rh_subject_r_map} -d ${r_map} -o ${rh_glm_stage1}_${outputname} 
		fsl_glm -i ${rh_subject_r_map} -d ${rh_glm_stage1}_${outputname} -m ${rh_thal_ds3} -o ${rh_glm_stage2}_${outputname} 
	done
