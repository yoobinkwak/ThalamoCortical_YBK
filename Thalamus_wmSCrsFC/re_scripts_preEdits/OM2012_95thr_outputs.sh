...
dir_thr_mc=OM2012_tensor_stats_outputs/tfce/fwe_corrected_p/mc_95thr_${input}
...
do
    ...
    if [ ! -e ${tstat1_mc} ] ; then
        ...
        zero=0
        if [ ! ${val} == ${zero} ] ; then
            echo ${tstat1_mc} >> OM2012_tensor_stats_outputs/tfce/fwe_corrected_p/survive_mc_95.txt
        fi
    fi
